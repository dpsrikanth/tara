...
middlewareObj.isActive = function (req, res, next) {
  if (req.isAuthenticated()) {
    ...
    if (req.user && req.user.status === "approved" || req.user.isAdmin === true) {
      return next();
    } else {
      req.flash(
        "error",
...
middlewareObj.isApprovedMemsaUser = function (req, res, next) {
  if (req.isAuthenticated()) {
    //console.log("IsAuth1");
    //console.log(req.user);
    if (req.user && req.user.status === "approved" && req.user.userType == 'memsa') {
      return next();
    } else {
      req.flash(
        "error",
...
middlewareObj.sanitizeUpdatedUserData = function (req, res, next) {
  req.body = {
    ...
    companyName: req.sanitize(req.body.companyName),
    address: {
      street1: req.sanitize(req.body.address["street1"]),
      street2: req.sanitize(req.body.address["street2"]),
      city: req.sanitize(req.body.address["city"]),
      province: req.sanitize(req.body.address["province"]),
      code: req.sanitize(req.body.address["code"]),
    },
    phone: req.sanitize(req.body.phone),
    website: req.sanitize(req.body.website),
    companyReg: req.sanitize(req.body.companyReg),
    vatNumber: req.sanitize(req.body.vatNumber),
    userType: req.sanitize(req.body.userType),
    mesmaId: req.sanitize(req.body.mesmaId)
  }
  return next();
};
